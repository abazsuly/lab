...
2. Upgrade all nodes using
```shell
sudo apt-get update
sudo apt-get upgrade
```

3. Install packages needed to use the Kubernetes apt repo on all nodes:
```shell
sudo apt-get install -y apt-transport-https ca-certificates curl gpg
```

4. Download public signing key for the Kubernetes package repo on all nodes. Create `/etc/apt/keyrings` if it does not exist.
```shell
curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.35/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
```

5. Add Kubernetes package repository on all nodes. Ensure that correct version is provided, this is command for v1.35:
```shell
echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.35/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list
```

6. Install Kubernetes packages and pin the version on all nodes:
```shell
sudo apt-get update
...
4. Enable kubelet via systemd on all nodes:
```shell
sudo systemctl enable kubelet
```

5. Install CRI (container runtime interface) on all nodes:
```shell
...
6. Enable IPv4 packet forwarding on all nodes
```shell
...
7. Configure `systemd` cgroup driver. 
...
8. On the controlplane node, Init the cluster using the appropriate `--pod-network-cidr` for the provided CNI. In this case, we will use the default flannel pod-network-cidr. Also, we need to pass in the IP that the node is available at, it might not automatically select the correct one. Pass it in via `--apiserver-advertise-address`:
```shell
sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --apiserver-advertise-address=192.168.40.69
```

9. Enable kubectl work for non-root user:
```bash
mkdir -p $HOME/.kube
...
10. Note down the `kubeadm join ...` command given by the init of the controlplane, this is how we add the worker nodes to the cluster: 
```shell
Then you can join any number of worker nodes by running the following on each as root:

kubeadm join 192.168.40.69:6443 --token q5ratc.38381twiak2rj2kj \
	--discovery-token-ca-cert-hash sha256:586a766b7e6009c341a997962bf4fdd6e98676c018dcababe27ca105ced16cb5
```

11. Before installing flannel, on all nodes, load the br_netfilter model and make it persistent across reboots:
```shell
sudo modprobe br_netfilter
echo br_netfilter | sudo tee /etc/modules-load.d/k8s.conf
```

12.  On all nodes, Set required sysctls and apply immediately, then restart containerd:
```shell
cat <<'EOF' | sudo tee /etc/sysctl.d/k8s.conf
...
13. Install flannel on controlplane node:
```shell
kubectl apply -f https://github.com/flannel-io/flannel/releases/latest/download/kube-flannel.yml
```

14. Run the join commands given by the controlplane on the worker nodes
    
15. All nodes should be present on controlplane via `kubectl get nodes` and all pods should be running via `kubectl get pods -A`
